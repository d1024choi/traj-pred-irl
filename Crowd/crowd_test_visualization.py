...
def test(input_args):
    ...
    for b in range(data_loader.num_test_scenes):
        ...
        for j in range(nps[0]):
            ...
            ADE_batch.append(displacement_error)
            FDE_batch.append(displacement_error[pred_len-1])
            cnt += 1

        ADE.append(ADE_batch)
        FDE.append(FDE_batch)
        NumPeds.append(nps)
        Pid_list.append(pid)
    ...
    f = open(save_dir, "wb")
    pickle.dump([GT_traj, EST_traj, NumPeds, ADE, FDE, Pid_list], f, protocol=2)
    f.close()
